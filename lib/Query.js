...
employeeQuery.joinQuery = `SELECT e.id, e.first_name, e.last_name, role.name as title, role.salary, department.name as department, CONCAT_WS(" ", m.first_name, m.last_name) AS manager
    FROM employee e
    LEFT JOIN employee m ON m.id = e.manager_id
    LEFT JOIN role ON e.role_id = role.id
    LEFT JOIN department ON role.department_id = department.id`;

roleQuery.joinQuery = `SELECT role.id, role.name, role.salary, department.name as department
    FROM role
